...
export async function GET(request: NextRequest) {
  const auth = requireAgentIntegration(request, "workout:read");
  if (!auth.ok) return auth.response;

  try {
    const data = await getAgentTodayWorkout(auth.context.userId);
    ...
    console.error("agent_tool_today_workout_failed", {
